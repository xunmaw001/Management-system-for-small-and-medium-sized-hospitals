...
<body>
		...
		<div id="contentText">
				...
				<div class="container mt-0">
					<div class="row breadcrumb-bar">
						<div class="col-md-6">
							<h3 class="block-title">编辑处方信息</h3>
						</div>
						<div class="col-md-6">
							<ol class="breadcrumb">
								...
								<li class="breadcrumb-item"><span>处方信息管理</span></li>
								<li class="breadcrumb-item active"><span>编辑处方信息</span></li>
							</ol>
						</div>
					</div>
				</div>
				...
				<div class="row">
					...
					<div class="col-md-12">
						<div class="widget-area-2 lochana-box-shadow">
							<h3 class="widget-title">处方信息信息</h3>
							<form id="addOrUpdateForm">
								<div class="form-row">
									<input id="updateId" name="id" type="hidden">
									<div class="form-group col-md-6">
										<label>处方单号</label>
										<input id="chufangdanhao" name="chufangdanhao" class="form-control" value="" readonly>
									</div>
									<div class="form-group col-md-6">
										<label>药品名称</label>
									...
									<div class="form-group col-md-6">
										<label>药品价格</label>
										<input id="yaopinjiage" name="yaopinjiage" class="form-control" onchange="calculation()">
									</div>
									<div class="form-group col-md-6">
										<label>数量</label>
									...
									<div class="form-group col-md-6">
										<label>出处方</label>
										<select id="chuchufangSelect" name="chuchufang" class="form-control">
												<option value=""></option>
												<option class="chuchufangOption" value="门诊">
													门诊
												</option>
												<option class="chuchufangOption" value="住院">
													住院
												</option>
										</select>	
									</div>	
	...
	<script>
		...
		function chufangdanhaoUuid(){
			if($("#chufangdanhao").val() == null || $("#chufangdanhao").val() == "null" || $("#chufangdanhao").val() == ""){
				$("#chufangdanhao").attr("value",Math.round(new Date().getTime()/1000));
			}
		}				
		  ...
		  beforeCreate: function(){
			  var id = window.sessionStorage.getItem("id");
				if(id != null && id != "" && id != "null"){
					$.ajax({ 
		                type: "GET",
		                url: baseUrl + "chufangxinxi/info/" + id,
				...
				if(crossrefid && crossuserid) {
					...
					http("chufangxinxi/page","GET",{
						...
						if(res.data.total >= crossoptnum){
							...
							httpJson("chufangxinxi/"+urlParam,"POST",data,(res)=>{
								if(res.code == 0){
		...
		function getDetails() {
			...
			if(id != null && id != "" && id != "null"){
				updateId = id;
				window.sessionStorage.removeItem('id');
				http("chufangxinxi/info/" + id,"GET",{},(res)=>{
					if(res.code == 0){
						ruleForm = res.data
						// 是/否下拉框回显
						setSelectOption();
		...
		function calculation(){
			...
			var y = 0;
			var flag = 0;
			if($("#yaopinjiage").val() != null && $("#yaopinjiage").val() != ""){
				if(x == 0 && flag < 2){
					x = (x+1) * parseFloat($("#yaopinjiage").val());
					flag++;
				}else{
					x = x * parseFloat($("#yaopinjiage").val());
					flag++;
				}
			}
		...
		function dataBind(){
			...
			$("#chufangdanhao").val(ruleForm.chufangdanhao);	
			...
			var chuchufangOptions = document.getElementsByClassName("chuchufangOption");
			for(var chuchufangCount = 0; chuchufangCount < chuchufangOptions.length;chuchufangCount++){
				if(chuchufangOptions[chuchufangCount].getAttribute('value') == ruleForm.chuchufang){
					chuchufangOptions[chuchufangCount].setAttribute('selected','selected');
				}
			}
		...
		if(crossTableName != null && crossTableName != '' && crossTableId != null && crossTableId != '' && crossTableId != -1){
			http(crossTableName + "/info/" + crossTableId,"GET",{},(res)=>{
				if(res.code == 0){
					...
					if(res.data != null && res.data != '' && res.data.chufangdanhao != null && res.data.chufangdanhao != ''){

						$("#chufangdanhao").val(res.data.chufangdanhao);
						$("#chufangdanhao").attr('readonly','readonly');
					}
					...
					if(res.data != null && res.data != '' && res.data.chuchufang != null && res.data.chuchufang != ''){

						var chuchufangOptions = document.getElementsByClassName("chuchufangOption");
						for(var chuchufangCount = 0; chuchufangCount < chuchufangOptions.length;chuchufangCount++){
							if(chuchufangOptions[chuchufangCount].getAttribute('value') == res.data.chuchufang){
								chuchufangOptions[chuchufangCount].setAttribute('selected','selected');
							}
						}
					}
